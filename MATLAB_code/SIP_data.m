...
fprintf("[Using Solar ONLY] Installed rated solar capacity:\t%.2f kWp\n", required_solar_rated_kWp)
...
fprintf("This corresponds to %.2f installed panels and %.2f MPPTs\n",num_panels_required,num_MPPTs_required)
fprintf("Each array has %.2f panels in series by %.2f in parallel\n",max_panel_series,max_panel_parallel)
...
battery_safe_cycle_fraction = 0.5;
single_battery_total_energy_kWh = 210;
single_battery_usable_energy_kWh = single_battery_total_energy_kWh * battery_safe_cycle_fraction;
...
% %fprintf("Load profile avg vs reservation annual consumptions (should be ~1.0 if I'm not screwing up): %.2f\n",test_val)
% Load_fig = figure;
% hold on
...
fprintf("Peak electricity consumption \t%.2f kW\n",max(load_week_profile_kW_seconds))
...
fraction_of_consumption_met_by_secondary = 0.75;
hours_per_year = 365.25 * 24;
hydropower_rated_kW = reservation_annual_electricity_requirement_kWh / hours_per_year * fraction_of_consumption_met_by_secondary;
...
fprintf("Estimated flowrate required:\t%.2f m3/s\n", hydropower_flowrate_m3s)
fprintf("Hydro-adjusted Solar Installed requirement %.2f kWp\n", required_solar_rated_kWp*(1-fraction_of_consumption_met_by_secondary))
%% Battery sizing
...
nighttime_consumption_kW = 400 %min(load_week_profile_kW_seconds);
...
weeks_of_autonomy = 1.0;

%Now redo the calculations for a battery pack to give a week of autonomy
%First estimate the generation conservatively
conservative_solar_capacity_factor = 0.11;
hours_per_week = 24*7;
conservative_weekly_solar_kWh = required_solar_rated_kWp * (1-fraction_of_consumption_met_by_secondary) * conservative_solar_capacity_factor * hours_per_week;
hydro_weekly_kWh = hydropower_rated_kW * hours_per_week;

autonomy_average_daily_deficit_kWh = reservation_weekly_electricity_consumption_kWh - hydro_weekly_kWh - conservative_weekly_solar_kWh;
battery_autonomy_capacity_kWh = autonomy_average_daily_deficit_kWh * weeks_of_autonomy;
peak_consumption_kW = 650 %max(load_week_profile_kW_seconds);
required_battery_autonomy_discharge_kW = peak_consumption_kW - hydropower_rated_kW;
num_batteries_required_autonomy = battery_autonomy_capacity_kWh / single_battery_usable_energy_kWh;days
total_battery_cost_autonomy_USD = num_batteries_required_autonomy * price_per_battery_USD;
battery_autonomy_discharge_kW = num_batteries_required_autonomy * single_battery_discharge_capacity_kW;

fprintf("\nFor overnight autonomy, battery-size:\nBattery Energy Capacity\t%.2f kWh\nBattery Discharge\t%.2f kW\n",battery_daily_capacity_kWh,battery_daily_discharge_kW)
fprintf("This corresponds to %.2f batteries\n\n", num_batteries_required_daily)
fprintf("\nFor %.1f weeks of autonomy from the grid, battery-size:\n Battery Energy Capacity %.2f kWh\n(required) Battery Discharge %.2f kWh\n",weeks_of_autonomy,battery_autonomy_capacity_kWh,required_battery_autonomy_discharge_kW)
fprintf("(actual) Battery charge/discharge rate limit %.2f kW \n",battery_autonomy_discharge_kW)
fprintf("This corresponds to %.2f batteries\n\n", num_batteries_required_autonomy)

%% Print cost breakdown
total_panel_cost_hydro_adjusted_USD = total_panel_cost_USD * (1-fraction_of_consumption_met_by_secondary);
total_cost_USD = total_panel_cost_hydro_adjusted_USD + total_hydropower_cost_USD + total_battery_cost_autonomy_USD + total_MPPT_cost_USD;
fprintf("\nCost Breakdown:\n\tCost\tItem\tPercentage of Budget\n")
fprintf("\t%.2f$ Panel cost\t%.2f\n",total_panel_cost_hydro_adjusted_USD,total_panel_cost_hydro_adjusted_USD/total_cost_USD*100)
fprintf("\t%.2f$   Hydropower cost\t%.2f\n",total_hydropower_cost_USD,total_hydropower_cost_USD/total_cost_USD*100)
fprintf("\t%.2f$   MPPT cost\t%.2f\n",total_MPPT_cost_USD,total_MPPT_cost_USD/total_cost_USD*100)
fprintf("\t%.2f$   Battery cost\t%.2f\n",total_battery_cost_autonomy_USD,total_battery_cost_autonomy_USD/total_cost_USD*100)