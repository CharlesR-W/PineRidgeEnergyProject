...
max_panel_series = np.floor(MPPT_Vmax_V / selected_module_Vmpp_V)
max_panel_parallel = np.floor(MPPT_Imax_A / selected_module_Impp_A)
...
num_panels_required = np.ceil(num_panels_required)
total_panel_cost_USD = num_panels_required * price_per_panel_USD

num_MPPTs_required = np.ceil(num_arrays_required)
price_per_MPPT = 700
...
test_val = np.mean(load_week_profile_kW_seconds)*(seconds_per_year/seconds_per_hour) / reservation_annual_electricity_requirement_kWh
...
#I do NOT take the np.ceil of this number, since these batteries are bespoke
